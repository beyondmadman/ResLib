...
CDllTestDlg::CDllTestDlg(CWnd* pParent /*=NULL*/)
	: CDialog(IDD_DLLTEST_DIALOG, pParent)
{
...
BOOL CDllTestDlg::OnInitDialog()
{
	...
	// TODO: 在此添加额外的初始化代码
	HMODULE hModule = LoadLibrary(_T("ResDll.dll"));
	if (hModule == NULL)
	{
		MessageBox(_T("加载资源文件失败！"));
		return true;
	}
	HBITMAP hBitmap = LoadBitmap(hModule, MAKEINTRESOURCE(IDR_BMP_HH));
